...
class QueriesGridCellProvider extends DataObjectGridCellProvider
{
    ...
    public function getTemplateVarsFromRowColumn($row, $column)
    {
        ...
        switch ($columnId) {
            ...
            case 'from':
                return ['label' => ($user ? $user->getUsername() : '&mdash;') . '<br />' . ($headNote ? DateManagement::date($datetimeFormatShort, strtotime($headNote->getDateCreated())) : '')];
            case 'lastReply':
                $latestReply = $notes->first();
                if ($latestReply && $latestReply->getId() != $headNote->getId()) {
                    $repliedUser = $latestReply->getUser();
                    return ['label' => ($repliedUser ? $repliedUser->getUsername() : '&mdash;') . '<br />' . DateManagement::date($datetimeFormatShort, strtotime($latestReply->getDateCreated()))];
                } else {
                    return ['label' => '-'];
                }
                // no break
